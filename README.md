...
- **视频标签屏蔽**：调 `/x/tag/archive/tags` 接口获取 `data[].tag_name`，按视频标签名屏蔽；音乐标签和话题标签不会展示或参与匹配
- **cm 软广**：屏蔽 `cm.bilibili.com` 链接
- **竖屏屏蔽**：按 API 分辨率判断竖屏
- **悬停临时显示**：遮挡卡片悬停指定秒后临时显示
- **自动连播处理**：播放页连播遇被屏蔽视频 → 切换 / 停止 / 不处理
- **用户空间页**：UP 名旁「屏蔽 / 已屏蔽」按钮 + 删除线 + 灰度
- **屏蔽原因可视化与一键取消**：被屏蔽的卡片显示具体屏蔽内容（UP 名 / 分类标签 / 软广 / 竖屏 / 广告 / 正则），
  且不再显示「屏蔽」按钮；点击可取消的原因按钮（UP 名精确匹配 / 分类标签）会**从黑名单删除对应规则**
  并只刷新该卡片重新判定——若还有其它屏蔽原因会继续屏蔽并刷新为新原因，没有则恢复显示。
  正则匹配（无法定位具体规则）、软广、竖屏、广告不支持取消，仅展示原因
- **分页初始化**：主页 / 搜索页 / 播放页 / 分类页 / 排行榜 / **动态页** / 用户空间页分别初始化
- **动态页（t.bilibili.com）**：以**整条动态**为屏蔽单位（视频投稿 / 转发 / 图文都算），
  发布者取 `.bili-dyn-title__text`、视频标题取 `.bili-dyn-card-video__title`；
  遮挡层与「屏蔽」按钮挂在动态里的视频卡上。注意：判断页面时**必须校验 hostname** ——
  `t.bilibili.com/` 的 pathname 也是 `/`，只按 pathname 会把它当主页（旧实现就是这样，等于没支持）
- **网络拦截（推荐/相关接口）**：命中 `urlPatterns` 的响应在流内过滤，**不额外发请求**：
  - UP 名精确 / 正则（与 DOM 侧同一套规则）；
  - **官方广告标记**：rcmd 条目的 `business_info` 非空即商业推广（实测含 `is_ad_loc` / `res_id` / `creative_type`），
    不再靠 `cm.bilibili.com` 链接猜；
  - **复用队列已缓存的分区/标签/竖屏数据**：该条目若已被队列判过（`view` / `/x/tag/archive/tags` 命中未过期缓存），
    就按分类标签、视频标签、竖屏规则一并过滤。流内字段实测（2026-09）**没有** `tid/tname`、`dimension`、视频标签，
    所以这三类只能靠缓存复用，绝不在这里新增请求（否则拖慢响应还可能触发限流）
- **管理面板**（五个标签页）：精确匹配(Up名字) / 正则匹配(Up/标题) / 屏蔽分类 / 屏蔽标签 / 插件配置，含「取消屏蔽 / 恢复屏蔽」、已屏蔽计数
  - **头部统计明细**（默认收起，点右上角箭头按钮展开）：除「已屏蔽视频 N」总数外，分三组列出明细 ——
    「屏蔽原因明细」UP/标题名、广告、CM 软广、分类标签、视频标签、竖屏；
    「网络与请求」网络拦截过滤条数、其中广告、改写响应次数、已判定卡片数、view 与视频标签接口的真实请求次数；
    「累计与趋势（按天持久化）」今日/近 7 天/累计屏蔽、今日/近 7 天拦截、累计判定 + **7 日趋势柱**。
    数值随判定/拦截实时刷新（DOM 只建一次，只改数值文本；收起时跳过数值写入，展开瞬间补一次）
  - **统计持久化**：内存计数器按天差值落盘（`blockStats`，保留 30 天明细 + all-time 汇总），
    所以刷新/换页后「今日 / 近 7 天 / 累计」仍在；面板显示的是"存储值 + 尚未刷盘的增量"，数字始终实时；
    插件配置页里有「清除」按钮可归零
  - **头部按钮**：`[展开/收起统计明细]`（中性圆形 + 会翻转的箭头）在左，`[关闭面板]`（圆形 `#fb7299` 底色 + 白色 SVG 叉图标，34px，悬停加深并轻微放大）在右
- **保留的优化**：视频页延迟 5 秒启用、标签名列表缓存（60 秒更新一次）、主页屏蔽后布局修正、
  **切到后台即暂停判定**（`isPageCurrentlyActive`）
- **切到后台暂停判定（有意设计，勿"优化"）**：页面不可见（`document.hidden`，含被其它窗口完全遮挡）时，
  卡片队列停在原地、卡片保持「未处理」的模糊遮盖，切回前台后自动继续。
  **原因**：同时开多个 B 站页面时，多页并发请求 `view` / `/x/tag/archive/tags` 会显著提高触发 B 站限流的概率；
  后台暂停等于把并发压回单页。**代价（刻意接受）**：后台页面里的卡片会一直糊着 —— 这是设计好的视觉状态，
  不是 bug，也不要改成"后台照常判定、只是不写 DOM"。
- **队列判定分两阶段**：先做零网络判定（软广链接 > UP主名精确 > 正则），命中即提交、不发请求也不限速；
  只有未命中的卡片才请求 `view` 接口做分类标签 / 竖屏判定，以及 `/x/tag/archive/tags` 接口做视频标签判定，并按 `200ms` 限速（防 API 限流）。
...
按标题/UP主名(`flagInfo`)、广告(`flagAD`)、分类标签(`flagTName`)、视频标签(`flagVideoTag`)、始终获取分类标签(`flagAlwaysFetchTName`)、cm软广(`flagCM`)、竖屏(`flagVertical`)、遮挡模式(`blockDisplayMode`，各类型可用 `displayModeInfo/AD/TName/VideoTag/CM/Vertical` 覆盖)、网络拦截(`flagNetworkIntercept`)、悬停临时显示(`flagHoverReveal`)、自动连播处理(`flagSkipBlockedAutoplay`) 等开关，以及标签缓存清除、悬停延迟、扫描间隔、竖屏阈值等设置。

其中 `flagAlwaysFetchTName`（默认开启）控制“已被 UP主名/正则/软广命中的卡片是否仍请求接口以显示分类标签按钮”：
开启时这些请求会排在低优先级的补标签队列，分类标签和视频标签始终可见且不拖慢其它卡片的判定；关闭则不再为这些卡片请求接口，
队列处理更快（搜索页翻页尤其明显），代价是这些卡片上看不到分类标签。

### 屏蔽原因与一键取消

被屏蔽的卡片上不再显示「屏蔽」按钮，而是显示具体屏蔽原因的按钮（如 `屏蔽原因: UP: xxx` / `屏蔽原因: 标签: xxx` / `屏蔽原因: 视频标签: xxx`）。
点击可取消的原因（仅 UP 名精确匹配、分类标签与视频标签支持）会：
1. 把对应规则从黑名单中删除（`exactMatchBlacklist` / `tagNameBlacklist` / `videoTagBlacklist`，同步持久化，面板列表同步刷新）；
2. 只刷新这一张卡片——恢复显示后重新入队做完整判定：若还有其它屏蔽原因（其它 UP 名规则 / 正则 /
   其它标签 / 软广 / 竖屏 / 广告）会继续屏蔽并刷新为新原因，没有则保持显示。

正则匹配（无法定位具体规则）、软广、竖屏、广告仅展示原因、不支持一键取消，请在面板中手动处理。
遮挡模式为「隐藏卡片」时卡片不可见，请先用面板「取消屏蔽」临时显示。
