...
被屏蔽的卡片上不再显示「屏蔽」按钮，而是显示具体屏蔽原因的按钮（如 `屏蔽原因: UP: xxx` / `屏蔽原因: 标签: xxx`）。
点击可取消的原因（仅 UP 名精确匹配与分类标签支持）会：
1. 把对应规则从黑名单中删除（`exactMatchBlacklist` / `tagNameBlacklist`，同步持久化，面板列表同步刷新）；
