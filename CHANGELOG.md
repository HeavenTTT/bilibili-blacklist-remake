...
- **管理面板头部统计明细**：在「已屏蔽视频 N」之外新增分两组明细 ——
  「屏蔽原因明细」UP/标题名、广告、CM 软广、分类标签、视频标签、竖屏各自分开；
  「网络与请求」网络拦截过滤条数、改写响应次数、已判定卡片数、view 与视频标签接口的真实请求次数。
  数值随判定/拦截实时刷新；DOM 只在建面板时创建一次，之后只改数值文本（`refreshBlockCountDisplay` 是高频调用）。
- **统计明细改为可折叠**：默认收起，头部右侧新增箭头按钮（位于关闭按钮左边）切换展开/收起（箭头随状态翻转、`aria-expanded` 同步）；
  收起期间跳过 11 个数值的写入，展开瞬间补一次刷新。
- **管理面板关闭按钮**：由纯文本「×」换成 34px 圆形 `#fb7299` 底色 + 白色 SVG 叉图标（悬停加深、轻微放大、带阴影），可点面积与辨识度都更大。
...
- **分区表增量更新（popular / ranking feed）**：新增 `updateTNameListFromFeed`，从热门/排行榜接口收集 `tidv2/tnamev2/pid_name_v2` 合并进分区缓存，可覆盖 channelKv 中还没有的新分区（如 2207 随拍·综合）；在处理队列为空时触发。channelKv 更新（`tLastTime`）与 feed 更新（`tFeedLastTime`）**各用独立的 12 小时节流**。
- **视频标签接口串行化**：读取顺序改为先 view（分类/竖屏）再 `/x/tag/archive/tags`（视频标签），不再并发；两个接口都校验 `code`，无返回时该卡片重试一次后放行。
- **视频标签过滤修正**：`normalizeVideoTagName` 统一要求 `tag_id` 与 `tag_name` 都存在才保留（兼容部分页面缺少 `tag_type/music_id` 字段的情况）。
- **管理面板**：宽度 500px → 600px（适配第 5 个标签页），新增 `backdrop-filter: blur(8px)` 毛玻璃效果与 `max-width: 80%` 小屏适配。
- **屏蔽列表搜索**：精确匹配 / 正则匹配 / 屏蔽分类 / 屏蔽标签 四个列表新增模糊搜索框（忽略大小写，无匹配显示占位）。
- **插件配置页按功能分组重排**：临时开关 / 屏蔽类型开关 / 分类数据与缓存 / 显示方式 / 交互 / 网络与性能 / 免责声明；分组标题加背景色区分。
- **标签汇总按钮 + 展开浮层**：卡片上不再逐个渲染标签按钮，只显示「分类 N / 标签 M」两个汇总按钮（数量为 0 则不显示）；悬停/点击对应按钮在卡片内弹出覆盖浮层查看该类别全部标签（自动滚动、悬停收起），彻底避免标签挤占/溢出卡片。
- **悬停节流/防抖**：展开浮层的显示做 300ms 节流（快速进出不重复 build/定位）、收起用 200ms 防抖，降低悬停时的渲染开销。
